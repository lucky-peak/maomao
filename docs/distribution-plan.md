...
- 包名: `@luckypeak-maomao/mcp-server`
- 版本: `0.1.0`
- Node 版本要求: `>=18.0.0`
...
```json
{
  "name": "@luckypeak-maomao/mcp-server",
  "version": "0.1.0",
...
```bash
# 全局安装（推荐用于 MCP 集成）
npm install -g @luckypeak-maomao/mcp-server

# 本地安装
npm install @luckypeak-maomao/mcp-server

# 使用 npx 直接运行（无需安装）
npx @luckypeak-maomao/mcp-server
```
